...
files only need to call ``set_worker_net_device(local_rank)``.
"""
...
def set_worker_gpu_nic_mapping(
    ...
    """Set UCX_NET_DEVICES from VLLM_GPU_NIC_PCIE_MAPPING for a single worker."""
    ...
    if local_rank not in pci_by_index:
        raise RuntimeError(
            f"No GPU PCI for NVML index {local_rank} in map "
    ...
    logger.info(
        "[rank %s] GPU PCI %s -> NIC PCI %s (%s) -> UCX_NET_DEVICES=%s",
        local_rank,
        gpu_bdf,
        nic_fmt,
        rdma_name,
...
def set_worker_net_device(local_rank: int) -> None:
    """Top-level entry point for both UniProcExecutor and MultiprocExecutor.

    Sets ``UCX_NET_DEVICES`` from ``VLLM_GPU_NIC_PCIE_MAPPING`` if present; no-op otherwise.
    """
