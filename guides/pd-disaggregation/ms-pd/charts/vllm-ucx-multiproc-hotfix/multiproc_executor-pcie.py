...
# --- UCX GPU→NIC PCIe mapping via VLLM_GPU_NIC_PCIE_MAPPING (TP-only / DP=1 on one node; see chart README) ---
...
def parse_gpu_nic_mapping(raw: str) -> dict[tuple[int, int, int, int], tuple[int, int, int, int]]:
...
def query_all_gpu_pci_bus_ids() -> dict[int, str]:
    """One nvidia-smi call: NVML GPU index -> pci.bus_id string."""
    proc = subprocess.run(
        [
            "nvidia-smi",
    ...
    for line in proc.stdout.strip().splitlines():
        ...
        pci_field = pci_part.strip()
        for tok in reversed(pci_field.replace(",", " ").split()):
            tok = tok.strip()
            try:
                normalize_pci(tok)
                out[idx] = tok
            ...
            raise RuntimeError(f"could not parse pci.bus_id from line: {line!r}")
    if not out:
        raise RuntimeError("nvidia-smi returned no GPU pci.bus_id rows")
    return out
...
def prefetch_gpu_pci_map_when_pcie_mapping_enabled() -> None:
    ...
    try:
        ...
        logger.info(
            "Prefetched GPU PCI map for UCX (%d GPUs) into %s",
            len(pci_map),
...
def rdma_name_for_nic_pci(nic_pci: tuple[int, int, int, int]) -> str:
    ...
    as the BDF string — that basename **is** the PCIe domain:bus:dev.fn address.

    Do **not** confuse with the **file** ``.../<bdf>/device`` inside that directory:
    that file holds the 16-bit PCI device id (e.g. ``0x101e``), not the BDF.
    Verify in a pod: ``readlink -f /sys/class/infiniband/mlx5_0/device``.

    ``VLLM_GPU_NIC_PCIE_MAPPING`` NIC keys must **normalize** (via ``normalize_pci``)
    to the same tuple as this basename (same format as ``nvidia-smi`` / sysfs uses for that NIC).
    """
...
def set_worker_gpu_nic_mapping(
    ...
    if local_rank not in pci_by_index:
        raise RuntimeError(
            f"No GPU PCI for NVML index {local_rank} in map (have indices {sorted(pci_by_index.keys())})"
        )
    gpu_bdf = pci_by_index[local_rank]
    ...
    rdma_name = rdma_name_for_nic_pci(nic_pci)
    # UCX IB/RoCE device suffix :1 — single-port NICs (typical modern Mellanox).
    ucx_dev = f"{rdma_name}:1"
    os.environ["UCX_NET_DEVICES"] = ucx_dev
    nic_fmt = f"{nic_pci[0]:04x}:{nic_pci[1]:02x}:{nic_pci[2]:02x}.{nic_pci[3]}"
    logger.info(
        "[rank %s] VLLM_GPU_NIC_PCIE_MAPPING: GPU PCI %s -> NIC PCI %s (%s) -> UCX_NET_DEVICES=%s",
        local_rank,
        gpu_bdf,
        nic_fmt,
        rdma_name,
...
class WorkerProc:
    ...
    def worker_main(*args, **kwargs):
        ...
        if os.environ.get("VLLM_GPU_NIC_PCIE_MAPPING", "").strip():
            ...
            set_worker_gpu_nic_mapping(local_rank)
