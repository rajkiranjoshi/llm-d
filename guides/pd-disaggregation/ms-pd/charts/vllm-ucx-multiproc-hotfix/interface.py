...
class Platform:
    ...
    def get_all_gpu_pci_bus_ids(cls) -> dict[int, str]:
        """Return a mapping from GPU index to its full PCI bus ID string.

        Platforms that support per-GPU RDMA NIC selection should override
        this method.  The base implementation unconditionally raises so
        that callers know the platform does not expose bus IDs.

        Returns:
            dict[int, str]: ``{gpu_index: "0000:XX:YY.Z", ...}``
        """
        raise NotImplementedError(
            f"{cls.__name__} does not implement get_all_gpu_pci_bus_ids"
        )
