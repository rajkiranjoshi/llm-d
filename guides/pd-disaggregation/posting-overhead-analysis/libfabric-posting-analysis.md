...
| Block size | 128 | 128 |
| Benchmark | MC=32, 300 reqs, OSL=1 | MC=32, 300 reqs, OSL=1 |

Raw benchmark logs and per-ISL KV transfer metrics are in [`logs/libfabric-efa/`](logs/libfabric-efa/) (EKS ISL sweep) and [`logs/ucx-ib/`](logs/ucx-ib/) (CoreWeave ISL sweep + ISL=24K standalone run).
