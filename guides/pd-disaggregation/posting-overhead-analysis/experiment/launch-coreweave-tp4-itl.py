...
on CoreWeave with TP4 configuration.

Measures the impact of posting overhead on ITL with MC=8, OSL=512 at ISL=4096 and ISL=24000.

Usage:
    tmux attach -t cursor-agent
    source ~/.env && python3 launch-coreweave-tp4-itl.py
"""
...
on CoreWeave with TP4 configuration. This measures how NIXL posting overhead impacts
...
- The TP4 CoreWeave values file exists at:
  guides/pd-disaggregation/ms-pd/values_coreweave_rdma.yaml
  (TP=4, rdma/ib=1, --max-model-len=32768, Llama-3.3-70B-Instruct-FP8-dynamic)
- helm, helmfile, just, kubectl are installed.
...
```bash
export KUBECONFIG=%(kubeconfig)s
kubectl -n rajjoshi-dev get pods -o wide
...
Check every 5 minutes. If pods crash or show OOM / max_model_len errors:
1. Edit ms-pd/values_coreweave_rdma.yaml
2. Try --max-model-len "24576" first, then "16384"
3. Re-run helmfile apply
4. If max-model-len drops below 24000, skip ISL=24000
...
```bash
cd %(workspace)s/guides/pd-disaggregation
OUTDIR="posting-overhead-analysis/benchmark-logs/itl-ucx-ib-tp4-$(date +%%Y%%m%%d-%%H%%M%%S)"
mkdir -p "$OUTDIR"
echo "Output: $OUTDIR"
```

### Step 6: Run ISL=4096 Benchmark (MC=8, 100 reqs, OSL=512)
...
```bash
kubectl -n rajjoshi-dev logs -l llm-d.ai/role=decode -c vllm --tail=20000 > "$OUTDIR/decode-isl4096.log" 2>&1
kubectl -n rajjoshi-dev logs -l llm-d.ai/role=prefill -c vllm --tail=20000 > "$OUTDIR/prefill-isl4096.log" 2>&1
grep "KV Transfer metrics" "$OUTDIR/decode-isl4096.log" > "$OUTDIR/kv-metrics-isl4096.txt" 2>/dev/null
```

### Step 7: Run ISL=24000 Benchmark (MC=8, 100 reqs, OSL=512)
```bash
export KUBECONFIG=%(kubeconfig)s
export NAMESPACE=rajjoshi-dev
cd %(workspace)s/guides/pd-disaggregation
...
After it completes, collect ALL logs:
```bash
kubectl -n rajjoshi-dev logs -l llm-d.ai/role=decode -c vllm --tail=20000 > "$OUTDIR/decode-isl24000.log" 2>&1
kubectl -n rajjoshi-dev logs -l llm-d.ai/role=prefill -c vllm --tail=20000 > "$OUTDIR/prefill-isl24000.log" 2>&1
grep "KV Transfer metrics" "$OUTDIR/decode-isl24000.log" > "$OUTDIR/kv-metrics-isl24000.txt" 2>/dev/null
```

### Step 8: Parse Results
For each ISL, extract ITL statistics from the benchmark output.
The vllm bench serve output contains lines like:
- Mean ITL (ms): ...
- Median ITL (ms): ...
- P99 ITL (ms): ...

Also extract KV transfer metrics (post_time, xfer_time) from the decode logs.
...
Verify cleanup:
```bash
kubectl -n rajjoshi-dev get pods
```
...
```
=== COREWEAVE TP4 ITL EXPERIMENT — FINAL REPORT ===

CONFIGURATION:
- Cluster: CoreWeave (UCX/IB, InfiniBand via shared device plugin rdma/ib=1)
...
COMPARISON WITH TP8 RESULTS:
TP8 ISL=4096:  Mean ITL=7.77ms, Median=7.78ms, P99=11.81ms
TP8 ISL=24000: Mean ITL=8.57ms, Median=8.71ms, P99=13.96ms
...
- The cluster uses OIDC auth with a cached refresh token.
...
print("=" * 60)
print("CoreWeave TP4 ITL Experiment — SDK Agent Launcher")
print("=" * 60)
